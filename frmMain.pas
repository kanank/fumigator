...
begin
  if not TCPClient.Connected then
    s := 'Соединение с сервером не установлено'
  else
  begin
    s := 'Соединение с сервером установлено';
    if not FisServerCmd then
      s := 'Соединение с сервером ограничено(!!!)'
  end;
  lblSocket.Caption := s;
end;
...
begin
  MsgBoxInformation(msgText, 'Сообщение от сервера');
end;
